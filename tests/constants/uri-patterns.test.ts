...
describe('URI Patterns Constants', () => {
    ...
    describe('SCHEMA_TABLES Pattern', () => {
      ...
      it('should capture schema name parameter correctly', () => {
        const testCases = [
          ...
          {
            uri: 'db://schemas/inventory_2024/tables',
            expected: 'inventory_2024',
          },
        ];
  ...
  describe('URI_TEMPLATES Constants', () => {
    ...
    it('should have type-safe template values (as const)', () => {
      ...
      const schemaTablesTemplate: 'db://schemas/{schemaName}/tables' =
        URI_TEMPLATES.SCHEMA_TABLES;
      const tableInfoTemplate: 'db://schemas/{schemaName}/tables/{tableName}' =
        URI_TEMPLATES.TABLE_INFO;
      const tableIndexesTemplate: 'db://schemas/{schemaName}/tables/{tableName}/indexes' =
        URI_TEMPLATES.TABLE_INDEXES;

      expect(schemaListTemplate).toBe('db://schemas');
      expect(schemaTablesTemplate).toBe('db://schemas/{schemaName}/tables');
      expect(tableInfoTemplate).toBe(
        'db://schemas/{schemaName}/tables/{tableName}'
      );
      expect(tableIndexesTemplate).toBe(
        'db://schemas/{schemaName}/tables/{tableName}/indexes'
      );
    });
    ...
    it('should follow hierarchical URI structure', () => {
      ...
      expect(URI_TEMPLATES.SCHEMA_LIST).toBe('db://schemas');
      expect(URI_TEMPLATES.SCHEMA_TABLES.startsWith('db://schemas/')).toBe(
        true
      );
      expect(URI_TEMPLATES.TABLE_INFO.startsWith('db://schemas/')).toBe(true);
      expect(URI_TEMPLATES.TABLE_INFO.includes('/tables/')).toBe(true);
      expect(URI_TEMPLATES.TABLE_INDEXES.startsWith('db://schemas/')).toBe(
        true
      );
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle special characters in schema and table names', () => {
      ...
      specialCases.forEach(
        ({ uri, pattern, expectedSchema, expectedTable }) => {
